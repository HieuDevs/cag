...
| Thông số (`jev-1.13.0`, tra ngày 2026-09-23) | Giá trị |
|---|---|
| Giá | $0.042 / 1M token input, output miễn phí |
| Độ trễ | 70–500ms |
| Context | 64K token mỗi request, trong đó 32K cho `state` cộng câu hỏi dài nhất |
| Rate limit | 250K token/giây, 1.200 request/phút (**đang được điều chỉnh liên tục**) |
| Ngôn ngữ | Tiếng Anh tốt nhất. Ngôn ngữ khác, kể cả chữ Hán, "xử lý được nhưng kém hơn" |
| Dữ liệu | Không dùng để train. ZDR chỉ có ở gói enterprise |
| SDK | `pip install typesafe-sdk`, `@typesafe-ai/sdk` |
...
jev = AsyncTypeSafeClient(model="jev-1.13.0")  # ghim version: ngưỡng confidence tune theo version
...
QUESTIONS = {
    ...
    "needs_context": Noul(
        instructions="Does the question refer to something earlier in the conversation, such as 'that word' or 'the sentence above'?"
    ),
}
...
async def route(question: str) -> Route:
    try:
        r = await asyncio.wait_for(
            jev.system_one(state={"question": question}, questions=QUESTIONS),
            timeout=0.8,
        )
    except Exception:
        return Route("large", False, "jev_unavailable")
    ...
    cacheable = cacheable and r.nouls["needs_context"].noul < 0.5
...
| Alias `jev-latest` tự đổi version | Ghim `jev-1.13.0`. Khi nâng version phải chạy lại eval và tune lại ngưỡng |
